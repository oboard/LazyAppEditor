...
import 'package:flutter/material.dart'
    show MaterialButton, Theme, ThemeData, Tooltip;
import 'package:flutter/rendering.dart';
import 'package:flutter/widgets.dart';
import 'package:lazyappeditor/lazyhtmlcore/src/widgets/button.dart';
...
class LazyWidgetFactory {
  ...
  Widget? buildButton(
          BuildMetadata meta, Widget child, GestureTapCallback onTap) =>
      LazyButton.filled(onPressed: onTap, child: child);
  ...
  Widget? buildImage(
      BuildContext context, BuildMetadata meta, ImageMetadata data) {
          ...
          Navigator.of(context).push(
            FadeRoute(builder: (context) {
              return PhotoViewGalleryScreen(
                images: [provider!], //传入图片list
                index: 0, //传入当前点击的图片的index
                heroTag: url, //传入当前点击的图片的hero tag （可选）
              );
            }),
          );
        },
  ...
  /// Called when the [AppletWidget]'s state is disposed.
  @mustCallSuper
  void dispose() {
    _flattener.dispose();
  }
  ...
  /// Calls [AppletWidget.onTapUrl] with [url].
  ///
  ...
  void parse(BuildContext context, BuildMetadata meta) {
    ...
    switch (meta.element.localName) {
      ...
      case kTagImg:
        _tagImg ??= TagImg(this, context).buildOp;
      ...
      case 'button': //addition!!!!!增加项
        meta[kCssDisplay] = kCssDisplayBlock;
        _tagButton ??= TagButton(this).buildOp;
        meta.register(_tagButton!);
        // meta
        //   ..[kCssBackgroundColor] = '#ff0'
        //   ..[kCssColor] = '#000';
        // meta.tsb.enqueue(_tagAColor ??= (tsh, _) => tsh.copyWith(
        //     style: tsh.style.copyWith(
        //         color: _widget?.hyperlinkColor ??
        //             tsh.getDependency<ThemeData>().highlightColor)));
  ...
  void reset(State state) {
    ...
    _widget = widget is AppletWidget ? widget : null;
  }

  /// Resolves full URL with [AppletWidget.baseUrl] if available.
